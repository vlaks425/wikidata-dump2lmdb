...
TABLE_NAMES = [
    'labels', 'descriptions', 'aliases', 'entity_values', 'entity_rels'
]
...
class Table:
    ...
    def write(self, json_value: List[Dict[str, Any]]):
        ...
        for json_obj in json_value:
            self.cur_file_writer.write(ujson.dumps(
...
class Writer:
    def __init__(self, path: Path, batch_size: int, total_num_lines: int):
        ...
        self.output_tables = {table_name: Table(
            path, batch_size, table_name) for table_name in TABLE_NAMES}
    ...
    def write(self, json_object: Dict[str, Any]):
        ...
        if self.cur_num_lines % 200000 == 0:
            ...
            estimated_time = time_elapsed * \
                (self.total_num_lines - self.cur_num_lines) / (200000*3600)
